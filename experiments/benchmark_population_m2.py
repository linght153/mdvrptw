"""M2 池内重组判别 runner (2026-09-07): 单实例 × 单 seed × 单臂, 独立进程。

用法: python experiments/benchmark_population_m2.py pr20 42 popcx_route|popcx_part
输出: results/population_m2/{inst}_s{seed}_{arm}.json
臂: popcx_route = pop + 路由级重组 | popcx_part = pop + 车场簇重组 (κ=0)
配对参照: pop = results/population_m1/{inst}_s{seed}_pop.json (M1 批, 不重跑)。
结构字段公式与 experiments/analyze_structure_diff.py analyze() 同款。
协议: docs/ma-alns-m2-prereg-20260907.md
"""
import json
import os
import sys
...
OUT = (Path(__file__).resolve().parent.parent
       / os.environ.get("MDVRPTW_RESULTS_ROOT", "results")) / "population_m2"
...
POP = {"population_mode": True, "population_size": 8,
       "population_tournament": 2, "population_dc_kappa": 0.0}

ARMS = {
    "popcx_route": dict(POP, population_cx_mode="route",
                        population_cx_rate=0.2, population_cx_inherit_prob=0.6),
    "popcx_part": dict(POP, population_cx_mode="partition",
                       population_cx_rate=0.2),
}
...
def structure_metrics(inst, sol):
    """车场分配几何 (公式 = analyze_structure_diff.analyze 同款, 2026-09-07)。"""
    nd = inst.num_depots
    ...
    for di, rl in sol.routes.items():
        for r in rl:
            for g in r:
                actual[g - nd] = di
    assert (actual >= 0).all()
    d_near = dm[:nd, nd:].min(axis=0)  # 每客户最近车场距离 (仅车场行 × 客户列)
