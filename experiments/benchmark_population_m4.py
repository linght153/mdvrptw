"""M4 双空间罚域 A/B runner (2026-09-07): 单实例 × 单 seed × 单臂, 独立进程。

用法: python experiments/benchmark_population_m4.py pr20 42 popcxgep_l1|popcxgep_l10
输出: results/population_m4/{inst}_s{seed}_{arm}.json
臂: popcxgep_l1 = popcxge + penalty tw mult=1 | popcxgep_l10 = mult=10 (κ=0)
配对参照: popcxge = population_m3/...popcxge.json (M3 批, 不重跑)。
协议: docs/ma-alns-m4-prereg-20260907.md
"""
import json
import os
import sys
...
OUT = (Path(__file__).resolve().parent.parent
       / os.environ.get("MDVRPTW_RESULTS_ROOT", "results")) / "population_m4"
...
_BASE_FORM = {"population_mode": True, "population_size": 8,
...
ARMS = {
    "popcxgep_l1": dict(_BASE_FORM, population_penalty_mode="tw",
                        population_penalty_mult=1.0),
    "popcxgep_l10": dict(_BASE_FORM, population_penalty_mode="tw",
                         population_penalty_mult=10.0),
}
